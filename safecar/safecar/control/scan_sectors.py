...
# UN R79 §5.6.4.8.2(ACSF Category C 감지 영역 그림)를 축소했다. 규정의 감지 영역은
# 부채꼴이 아니라 **옆 차로를 따라 뒤로 뻗은 직사각형**이다(자기 차로 뒤는 포함 안 함).
# 좌표는 라이다 기준 REP-103: x 전방+, y 좌측+ (우측은 y<0).
#
# 차가 길이·폭을 똑같은 비율로 줄인 모형이 아니라서 축별로 따로 축소한다
# (기준 차량 아반떼 CN7: 전장 4,710 mm, 전폭 1,825 mm / 본 차량 440 × 380 mm).
# - 뒤(종방향): R79 S_rear ≥ 55 m(§5.6.4.8.1) × 전장비 440/4710(≈1/10.7) = 5.1 m
# - 옆(횡방향): 트랙 차로 1개 폭 ≈ 차폭 × 2 = 0.76 m (실측). R79는 감지 범위가
#   "바로 옆 차로로의 변경이 위험하지 않은지 판단하기에 충분"해야 한다고 하고
#   (§5.1.6.3.9.17), ISO 17387도 인접 구역을 "옆 차로를 덮는 영역"으로 정의한다(§2.4).
#   그림의 6 m를 차로폭 비율(0.76/3.5)로 그대로 줄이면 1.3 m가 되는데, 그러면 옆
#   차로 너머의 인도·연석(2026-09-22 실측 1.25 m)까지 잡혀 항상 자차로정차가 된다.
ZONE_SIDE_M = 0.76
ZONE_REAR_M = 5.1
...
def _self_check():
    ...
    # 우측 차로 영역: box = x -5.32~0.22, y -0.95~-0.19
    box = right_lane_zone()
    assert all(abs(a - b) < 1e-9 for a, b in zip(box, (-5.32, 0.22, -0.95, -0.19))), box
