...
| `vehicle_half_width_m` | 0.15 | 라이다 중심 ~ 차체 우측면. **추정값, 줄자 실측 필요**(3-4) |
| `vehicle_front_m` / `vehicle_rear_m` | 0.15 / 0.15 | 라이다 중심 ~ 차체 앞끝/뒤끝. **추정값, 실측 필요** |
...
**차체 치수** — 라이다 회전 중심에서 줄자로 재서 `vehicle_half_width_m`(우측면까지),
`vehicle_front_m`(앞끝까지), `vehicle_rear_m`(뒤끝까지)에 넣습니다. 너무 작게 넣으면 차체
