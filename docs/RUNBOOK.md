...
```
/scan 끊김(센서 blindness)?         ─예─▶ 자차로정차   (UN R79 §5.6.4.8.4)
     │아니오
우측 차로 감지 영역에 물체가 있나?  ─예─▶ 자차로정차   (UN R79 §5.1.6.3.9.2)
     │아니오
                                   └─▶ 우차로정차
```

차선(흰선/노란선)을 놓치는 경우는 결정 시점이 아니라 실행 중에 처리합니다 — 놓친 동안은
조향을 끊고 직진합니다(직진정차와 같은 동작).

**우측 차로 감지 영역** — UN R79(ACSF Category C / RMF) §5.6.4.8.2의 감지 영역을
축소했습니다(길이는 아반떼 CN7 전장 대비 440/4,710, 폭은 트랙 차로 1개). 규정의 감지 영역은 부채꼴이 아니라 **옆 차로를 따라 뒤로 뻗은 직사각형**이고,
자기 차로 뒤는 포함하지 않습니다.

```
          ↑ 전방
       ┌─────┐
       │ 차  │▒▒▒▒▒▒   ← 옆: 차체 우측면부터 0.76m (트랙 차로 1개 = 차폭 × 2)
       │     │▒▒▒▒▒▒
       └─────┘▒▒▒▒▒▒
              ▒▒▒▒▒▒
              ▒▒▒▒▒▒   ← 뒤: 차체 뒤끝부터 5.1m  (R79 S_rear 55m × 440/4710)
              ▒▒▒▒▒▒
```

R79 §5.6.4.7은 뒤차의 **속도**로 임계거리를 계산하지만, 라이다 한 장으로는 접근 속도를
모릅니다. 그래서 영역 안에 **무엇이든 있으면** 다가오는 차로 간주하고 차선변경을 포기합니다
(규정보다 보수적).

| 파라미터 | 기본값 | 역할 |
|---|---|---|
| `mrm_zone_side_m` | 0.76 | 감지 영역 폭 = 트랙 차로 1개 (R79 §5.1.6.3.9.17) |
| `mrm_zone_rear_m` | 5.1 | 감지 영역 길이, 차체 뒤끝부터 (R79 55m × 전장비 440/4710) |
| `vehicle_half_width_m` | 0.19 | 라이다 중심 ~ 차체 우측면 (실측 폭 380 mm의 절반) |
| `vehicle_front_m` / `vehicle_rear_m` | 0.22 / 0.22 | 라이다 중심 ~ 차체 앞끝/뒤끝 (실측 길이 440 mm의 절반, 라이다가 정중앙이라고 가정) |
...
| 파라미터 | 기본값 | 역할 |
|---|---|---|
| `mrm_lateral_bias` | 0.5 | 0이 아니면 우차로정차 시도, `0.0`이면 무조건 자차로정차(R157 기본) |
| `mrm_speed_ratio` | 0.85 | 대피 중 속도 비율. 0.12 m/s 밑으로 떨어지면 차가 안 움직임 |
| `mrm_align_offset` | 0.05 | 노란선이 화면 중앙(파란 기준선)에 이만큼 가까워지면 정지 시작 |
| `mrm_stop_duration` | 3.0 | 정지 시작 후 속도 0까지(초), 두 모드 공통. 0.255 m/s에서 약 0.4m 더 굴러간다 |
| `mrm_max_duration` | 6.0 | 이 시간 안에 못 붙으면 그 자리에서 정지 시작 (안전판) |
| `mrm_transition_time` | 3.0 | 자차로정차의 감속 구간(초) |
...
| `emergency_stop_m` | 1.0 | 전방에 이보다 가까우면 **종류 무관** 정지 (박스·콘처럼 Hailo가 모르는 물체). 0.3은 0.15 m/s 기준이라 0.3 m/s 주행에서 삼각콘 앞에 못 섰다. 곡선에서 벽에 오정지가 잦으면 0.6으로 |
...
> ⚠️ 차 우측 0.76m 안이나 우후방 5.1m 안(우측 차로 감지 영역)에 벽·사람·물건이 있으면
> **항상 `자차로정차`** 가 나옵니다. 버그가 아닙니다. 트리거 전에 `:8080`의 `RIGHT LANE`
> 줄이 `clear`인지 먼저 보세요.

### 3-4. 라이다 각도·차체 치수 실측 (한 번만)

`ydlidar.yaml`에 `reversion: true`가 걸려 있어 **ROS 표준 각도와 다를 수 있습니다.**
우측 차로 감지 영역은 ROS 표준(REP-103: 전방 0°, 좌측 +90°, 우측 -90°, 후방 ±180°)을
가정하므로, 각도가 다르면 영역이 엉뚱한 곳을 봅니다.
...
2. **차 우측에만 물체** → `<== 최근접` 칸의 중심각이 **-90°**여야 함
3. **뒤에만 물체** → **±180°**여야 함
4. 전방 섹터 `obstacle_front_deg`도 같은 방법(**0°**여야 함)

2026-09-22 실측: 우측 -90°, 후방 -180° — 표준과 일치.

**차체 치수** — 2026-09-22 실측 폭 380 mm × 길이 440 mm, 라이다가 차체 정중앙에 있다고
가정해 기본값을 0.19 / 0.22 / 0.22 m로 넣었습니다. 라이다가 앞뒤로 치우쳐 있으면 라이다 회전
중심에서 줄자로 다시 재서 `vehicle_front_m`(앞끝까지), `vehicle_rear_m`(뒤끝까지)를
고치세요. 너무 작게 넣으면 차체
자체가 영역에 잡혀 항상 자차로정차가 나옵니다(`:8080`의 `RIGHT LANE`이 빈 곳에서도
`object`면 이 경우).
