...
**우측 차로 감지 영역** — UN R79(ACSF Category C / RMF) §5.6.4.8.2의 감지 영역을 1/10로
축소했습니다. 규정의 감지 영역은 부채꼴이 아니라 **옆 차로를 따라 뒤로 뻗은 직사각형**이고,
자기 차로 뒤는 포함하지 않습니다.

```
          ↑ 전방
       ┌─────┐
       │ 차  │▒▒▒▒▒▒   ← 옆: 차체 우측면부터 0.6m  (R79 S_sensor,side = 6m)
       │     │▒▒▒▒▒▒
       └─────┘▒▒▒▒▒▒
              ▒▒▒▒▒▒
              ▒▒▒▒▒▒   ← 뒤: 차체 뒤끝부터 5.5m    (R79 S_rear 최소 55m)
              ▒▒▒▒▒▒
```
...
| 파라미터 | 기본값 | 역할 |
|---|---|---|
| `mrm_zone_side_m` | 0.6 | 감지 영역 폭 (R79 6m × 1/10) |
| `mrm_zone_rear_m` | 5.5 | 감지 영역 길이, 차체 뒤끝부터 (R79 55m × 1/10) |
...
> ⚠️ 차 우측 0.6m 안이나 우후방 5.5m 안(우측 차로 감지 영역)에 벽·사람·물건이 있으면
